...
def mutations_gyrA(ref,ech,ech_out_1,ech_out_2):
    ...
    for i in dico_S83:
        muta_S83[i] = round(float(dico_S83[i]/muta)*100,2)
    for i in dico_D87:
        muta_D87[i] = round(float(dico_D87[i]/muta)*100,2)
    dic_muta_aa_posi={"Decalage" : round(float(decalage/muta)*100,2), "S83" : muta_S83,"D87" : muta_D87}
    
    print(dic_muta_aa_posi)
    print(reads)
    df2 = pd.DataFrame(data=muta_S83, index=[0])
    df2 = (df2.T)
    df2.to_excel(ech_out_1)
    df3 = pd.DataFrame(data=muta_D87, index=[0])
    df3 = (df3.T)
