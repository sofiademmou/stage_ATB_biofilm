...
f = function(file,excel=TRUE){
  ...
  for (i in 1:84){
    ...
    tab$Dilution[i]=str_sub(tab$Sample[i],start=4) #on récupère les 2 dernier caractères correspondant à la dilution
...
tri_qPCR = function(tab,Y.intercept,slope,gammeNTC=FALSE,LOQ,moyLOQ,target=FALSE,range=FALSE,ref){ 
  ...
  #Si on a des NTC avec des Cq (pas NA), et des pics au même endroit que les Tm STD => on crée une liste avec les numeros de ligne qui vérifient cette condition
  if (gammeNTC=="NTC"){  
...
# Analyse statistique
### Plot des abondance absolue (SQ gène dilués)

```{r plots, warning=FALSE, include=FALSE}
metadata = read.csv("metadata_qPCR.csv", row.names=1, sep=";")

metadata$type_ech = factor(metadata$type_ech, levels=c('BF in situ','Eaux usées','BF in vitro'))

results_log_plot = function(tab,metadata,PASS=TRUE,moy,std){
  for (i in 1:20){
  ...
  tab_lm_AB = subset(tab_lm, Content=="Unkn" & Campain!="F" & Campain!="E")
  # tab_lm_AB = subset(tab_lm, Content=="Unkn" & Campain!="F" & Campain!="E")
  tab_lm_EF = subset(tab_lm, Content=="Unkn" & Campain!="A" & Campain!="B")
  nrow(tab_lm_AB)
  nrow(tab_lm_EF)
  
  if (PASS==TRUE){
    
    means=aggregate(log_absolu~Mat_Camp, data = tab_lm_AB, FUN = function(x) {round(mean(x), digits=2)})
    std=aggregate(log_absolu~Mat_Camp, data = tab_lm_AB, FUN = function(x) {round(sd(x), digits=2)})

  #J7 et J14 ensemble
  p1=ggplot(tab_lm_AB) +
    aes(x=Material, y=log_absolu,color=Material) +
    geom_boxplot(outlier.colour="black", outlier.shape=NA,outlier.size=2, notch=FALSE) +
    scale_color_manual(values=c("black","#999999","red","#f1c40f","#ffffff00","black", "#DEB887"))+
    ylim(-1, 10) +
    ylab("Nombre de copies de gènes/unité de quantité \n (g pour BF in situ, mL pour eaux usées, coupons pour BF in vitro)") +
    geom_dotplot(aes(fill=Inoculum,color=Tm_QC), binaxis='y', stackdir='center',size=1, position=position_dodge(0)) +
    scale_fill_manual(values=c("#196f3d", "#1abc9c", "#2471a3", "#56B4E9")) +
    geom_hline(aes(yintercept = -6.5),color="red",linetype = "dashed") +
    # geom_text(aes(x=Material, y=log_absolu,label=ID), size=1,inherit.aes = FALSE,check_overlap = TRUE) +
    facet_grid(. ~ type_ech, scales="free_x", space = "free_x") +
    labs(title = tab$Target[1],
              subtitle = "Effet de l'inoculum et du matériel sur la quantité de gène de résistance",
              caption = paste("µ ech envt : ", means$log_absolu[1], "+/-", std$log_absolu[1], " (BF=", means$log_absolu[2], ")", "\n",
                                "µ coupon béton (C) : ", means$log_absolu[3], "+/-", std$log_absolu[3], "\n",
                                "µ coupon polycarbonate (P) : ", means$log_absolu[4], "+/-", std$log_absolu[4],
                                sep = " ")
           ) + 
    theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5), plot.caption = element_text(size=7, color="dimgrey", face="italic"), axis.title.y = element_text(size = 8))
  
  print(p1)

  
  
  tab_lm_EF$campaign = factor(tab_lm_EF$campaign,levels = c("envt","F (low)","E (high)"))

    means_ATB=aggregate(log_absolu~ATB_type_plot, data = tab_lm_EF, FUN = function(x) {round(mean(x), digits=2)})
    std_ATB=aggregate(log_absolu~ATB_type_plot, data = tab_lm_EF, FUN = function(x) {round(sd(x), digits=2)})
    # 
    # means_envt=aggregate(log_absolu~campaign, data = tab_lm_EF, FUN = function(x) {round(mean(x), digits=2)})
    # std_envt=aggregate(log_absolu~campaign, data = tab_lm_EF, FUN = function(x) {round(sd(x), digits=2)})
      
    means_camp=aggregate(log_absolu~campaign, data = tab_lm_EF, FUN = function(x) {round(mean(x), digits=2)})
    std_camp=aggregate(log_absolu~campaign, data = tab_lm_EF, FUN = function(x) {round(sd(x), digits=2)})
  
  p2=ggplot(tab_lm_EF) +
    aes(x=ATB_type, y=log_absolu,color=ATB_type) +
    geom_boxplot(outlier.colour="black", outlier.shape=NA,outlier.size=2, notch=FALSE) +
    scale_color_manual(values=c("#999999","red", "#f1c40f", "#e67e22", "#c0392b","black"))  +
    ylim(-1, 10) +
    ylab("Nombre de copies de gènes/unité de quantité \n (g pour BF in situ, mL pour eaux usées, coupons pour BF in vitro)") +
    geom_dotplot(aes(fill=campaign,color=Tm_QC), binaxis='y', stackdir='center',size=1, position=position_dodge(0)) +
    scale_fill_manual(values=c("#ecf0f1", "#999999", "#2c3e50")) + 
    geom_hline(aes(yintercept = -6.5),color="red",linetype = "dashed") +
    # geom_text(aes(x=ATB_type, y=log_absolu,label=ID), size=1,inherit.aes = FALSE) +
    facet_grid(. ~ type_ech, scales="free_x", space = "free_x") +
    labs(title = tab$Target[1],
              subtitle = "Effet du type d'ATB sur la quantité de gène de résistance (campagnes EF)",
              caption = paste("µ ech envt : ", means$log_absolu[1], "+/-", std$log_absolu[1], " (BF=", means$log_absolu[2], ")", "\n",
                                "µ sans ATB (0) : ", means_ATB$log_absolu[1], "+/-", std_ATB$log_absolu[1], "\n",
                                "µ ajout FQ1 : ", means_ATB$log_absolu[2], "+/-", std_ATB$log_absolu[2], "\n",
                                "µ ajout FQ2 : ", means_ATB$log_absolu[4], "+/-", std_ATB$log_absolu[4], "\n",
                                "µ ajout FQ1+FQ2 : ", means_ATB$log_absolu[5], "+/-", std_ATB$log_absolu[5],
                                sep = " ")
           ) + 
    theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5), plot.caption = element_text(size=7, color="dimgrey", face="italic"), axis.title.y = element_text(size = 8))
  ...
  p3=ggplot(tab_lm_EF) +
    aes(x=campaign, y=log_absolu,color=campaign) +
    geom_boxplot(outlier.colour="black", outlier.shape=NA,outlier.size=2, notch=FALSE) +
    scale_color_manual(values=c("#2c3e50","#CAD5D8", "#999999", "red", "black"))  +
    ylim(-1, 10) +
    ylab("Nombre de copies de gènes/unité de quantité \n (g pour BF in situ, mL pour eaux usées, coupons pour BF in vitro)") +
    geom_dotplot(aes(fill=ATB_type,color=Tm_QC), binaxis='y', stackdir='center',size=1, position=position_dodge(0)) +
    scale_fill_manual(values=c("#999999", "#f1c40f", "#e67e22", "#c0392b","black")) + 
    geom_hline(aes(yintercept = -6.5),color="red",linetype = "dashed") +
    # geom_text(aes(x=ATB_type, y=log_absolu,label=ID), size=1,inherit.aes = FALSE) +
    facet_grid(. ~ type_ech, scales="free_x", space = "free_x") +
    labs(title = tab$Target[1],
              subtitle = "Effet du dosage sur la quantité de gène de résistance (campagnes EF)",
              caption = paste("µ ech envt : ", means$log_absolu[1], "+/-", std$log_absolu[1], " (BF=", means$log_absolu[2], ")", "\n",
                                "µ campagne E : ", means_camp$log_absolu[3], "+/-", std_camp$log_absolu[3], "\n",
                                "µ campagne F : ", means_camp$log_absolu[2], "+/-", std_camp$log_absolu[2],
                                sep = " ")
           ) +  
    theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5), plot.caption = element_text(size=7, color="dimgrey", face="italic"), axis.title.y = element_text(size = 8))
...
##############################################################################################################
  ...
  p1=ggplot(tab_lm_AB) +
    aes(x=Material, y=log_absolu,color=Material) +
    geom_boxplot(outlier.colour="black", outlier.shape=NA,outlier.size=2, notch=FALSE) +
    scale_color_manual(values=c("black","#999999","#f1c40f","#ffffff00","black", "#DEB887"))+
    ylim(-1, 10) +
    ylab("Nombre de copies de gènes/unité de quantité \n (g pour BF in situ, mL pour eaux usées, coupons pour BF in vitro)") +
    geom_dotplot(aes(fill=Inoculum,color=Tm_QC), binaxis='y', stackdir='center',size=1, position=position_dodge(0)) +
    scale_fill_manual(values=c("#196f3d", "#1abc9c", "#2471a3", "#56B4E9")) +
    ...
    theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5), plot.caption = element_text(size=7, color="dimgrey", face="italic"), axis.title.y = element_text(size = 8))  #) #couleur des points en fonction de l'inoculum mais meme boxplot
  ...
  p2=ggplot(tab_lm_EF) +
    aes(x=ATB_type, y=log_absolu,color=ATB_type) +
    geom_boxplot(outlier.colour="black", outlier.shape=NA,outlier.size=2, notch=FALSE) +
    scale_color_manual(values=c("#999999", "#f1c40f", "#e67e22", "#c0392b","black"))  +
    ylim(-1, 10) +
    ylab("Nombre de copies de gènes/unité de quantité \n (g pour BF in situ, mL pour eaux usées, coupons pour BF in vitro)") +
    geom_dotplot(aes(fill=campaign,color=Tm_QC), binaxis='y', stackdir='center',size=1, position=position_dodge(0)) +
    scale_fill_manual(values=c("#ecf0f1", "#999999", "#2c3e50")) + 
  ...
  p3=ggplot(tab_lm_EF) +
    aes(x=campaign, y=log_absolu,color=campaign) +
    geom_boxplot(outlier.colour="black", outlier.shape=NA,outlier.size=2, notch=FALSE) +
    scale_color_manual(values=c("#2c3e50","#CAD5D8", "#999999", "black"))  +
    ylim(-1, 10) +
    ylab("Nombre de copies de gènes/unité de quantité \n (g pour BF in situ, mL pour eaux usées, coupons pour BF in vitro)") +
    geom_dotplot(aes(fill=ATB_type,color=Tm_QC), binaxis='y', stackdir='center',size=1, position=position_dodge(0)) +
    scale_fill_manual(values=c("#999999", "#f1c40f", "#e67e22", "#c0392b","black")) + 
...
stat_summary(fun = mean, geom = "text", size=2, col="coral2", yjust=-15, aes(label= paste(round(..y..,digits=3))))
...
### 16S  
```{r 16S, include=FALSE}
# png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/qnrA_plot_AB.png",sep=""),width=8,height = 4, units='in',res=600)
png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/figures_finales/absolu/16S_plot_AB_absolu.png",sep=""),width=6,height = 4, units='in',res=600)
results_log_plot(PCR_18_16S_tri,metadata,PASS=FALSE)[1]
dev.off()

png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/figures_finales/absolu/16S_plot_ATB_EF_absolu.png",sep=""),width=6,height = 4, units='in',res=600)
results_log_plot(PCR_18_16S_tri,metadata,PASS=FALSE)[2]
dev.off()

png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/figures_finales/absolu/16S_plot_Dosage_EF_absolu.png",sep=""),width=6,height = 4, units='in',res=600)
results_log_plot(PCR_18_16S_tri,metadata,PASS=FALSE)[3]
dev.off()
```
</br>
</br>
...
# png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/qnrA_plot_AB.png",sep=""),width=8,height = 4, units='in',res=600)
png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/figures_finales/absolu/qnrA_plot_AB_absolu.png",sep=""),width=6,height = 4, units='in',res=600)
results_log_plot(PCR_19_qnrA_tri,metadata)[1]
dev.off()

png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/figures_finales/absolu/qnrA_plot_ATB_EF_absolu.png",sep=""),width=6,height = 4, units='in',res=600)
results_log_plot(PCR_19_qnrA_tri,metadata)[2]
dev.off()

png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/qnrA_plot_Dosage_EF_absolu.png",sep=""),width=6,height = 4, units='in',res=600)
...
# png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/qnrB_plot_AB_ID.png",sep=""),width=6,height = 4, units='in',res=600)
png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/figures_finales/absolu/qnrB_plot_AB_absolu.png",sep=""),width=6,height = 4, units='in',res=600)
results_log_plot(PCR_20_qnrB_tri,metadata)[1]
dev.off()

png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/qnrB_plot_ATB_EF_absolu.png",sep=""),width=6,height = 4, units='in',res=600)
results_log_plot(PCR_20_qnrB_tri,metadata)[2]
dev.off()

png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/figures_finales/absolu/qnrB_plot_Dosage_EF_absolu.png",sep=""),width=6,height = 4, units='in',res=600)
...
png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/figures_finales/absolu/qnrS_plot_AB_absolu.png",sep=""),width=6,height = 4, units='in',res=600)
results_log_plot(PCR_21_qnrS_tri,metadata,PASS=FALSE)[1]
dev.off()

png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/figures_finales/absolu/qnrS_plot_ATB_EF_absolu.png",sep=""),width=6,height = 4, units='in',res=600)
results_log_plot(PCR_21_qnrS_tri,metadata,PASS=FALSE)[2]
dev.off()

png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/qnrS_plot_Dosage_EF_absolu.png",sep=""),width=6,height = 4, units='in',res=600)
...
png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/figures_finales/absolu/qnrD_plot_AB_absolu.png",sep=""),width=6,height = 4, units='in',res=600)
results_log_plot(PCR_22_qnrD_tri,metadata,PASS=FALSE)[1]
dev.off()

png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/qnrD_plot_ATB_EF_absolu.png",sep=""),width=6,height = 4, units='in',res=600)
results_log_plot(PCR_22_qnrD_tri,metadata,PASS=FALSE)[2]
dev.off()

# png(paste("C:/Users/Marie Claire Demmou/OneDrive/stage/ENVT/qPCR/figures_qnr/figures_finales/absolu/qnrD_plot_Dosage_EF_absolu.png",sep=""),width=6,height = 4, units='in',res=600)
png(paste("C:/Users/sdemmou/OneDrive/stage/ENVT/qPCR/figures_qnr/figures_finales/absolu/qnrD_plot_Dosage_EF_absolu.png",sep=""),width=6,height = 4, units='in',res=600)
