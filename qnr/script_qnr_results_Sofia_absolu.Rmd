...
```{r fonction_results, echo=TRUE, warning=FALSE}
results_log = function(tab,metadata,testNormalite=FALSE,extendedResults=FALSE){
  ...
  cat('\n')
  cat(paste("\t","******","Facteurs significatifs","******",sep=""),'\n\n')
  ...
  C=as.factor(tab_lm_AB$Campain)
  Jour=as.factor(tab_lm_AB$Day)
  AB.log= tab_lm_AB$log_absolu
  if (testNormalite==TRUE){
     p1=ggplot(tab_lm_AB, aes(x=ID,y=log_absolu,label=ID)) + geom_point(size=0.5) + geom_text(size=2.5) ; p2= ggplot(tab_lm_AB, aes(x=log_absolu,label=ID)) + geom_histogram() ; plot_grid(p1, p2)
  }
  ...
  if (extendedResults==TRUE){
    model.AB.dunnett <- aov(log_absolu~Inoculum, data = tab_lm_AB_dunnett)
    shapiro.test(model.AB.dunnett$residuals) #si non normalité des résidus on fait Dunn test ?
    print(summary(model.AB.dunnett))
    print(kruskal.test(log_absolu~Inoculum, data = tab_lm_AB_dunnett))
  }
  print(ggplot(tab_lm_AB_dunnett, aes(x=type_ech, y=log_absolu,color=type_ech)) +
    geom_boxplot(outlier.colour="black", outlier.shape=16,outlier.size=2, notch=FALSE) +
    scale_color_manual(values=c("#196f3d", "#1abc9c", "#56B4E9", "#2471a3")) +
    geom_dotplot(binaxis='y', stackdir='center',
                   position=position_dodge(1)))
  ...
  #Dunn test non paramétrique
  print(dunnTest(log_absolu ~ type_ech,
           data=tab_lm_AB_dunnett,
           method="bonferroni"))
  
  #print(tab_lm_AB_dunnett$log_absolu[which(tab_lm_AB_dunnett$type_ech=="BF in vitro")])
  print(t.test(tab_lm_AB_dunnett$log_absolu[which(tab_lm_AB_dunnett$type_ech=="BF in vitro")], alternative="two.sided", mu = tab_lm_AB_dunnett$log_absolu[which(tab_lm_AB_dunnett$type_ech=="BF in situ")], conf.level = 0.95))
  print(t.test(tab_lm_AB_dunnett$log_absolu[which(tab_lm_AB_dunnett$type_ech=="Eaux usées")], alternative="two.sided", mu = tab_lm_AB_dunnett$log_absolu[which(tab_lm_AB_dunnett$type_ech=="BF in situ")], conf.level = 0.95))
  ...
  FQ2=as.factor(tab_lm_EF$FQ2) #présence absence
  #J=as.factor(tab_lm_EF$Day) #J14 pour tous les échantillons donc on peut supprier ce facteur
  EF.log=tab_lm_EF$log_absolu
  if (testNormalite==TRUE){
     p1=ggplot(tab_lm_EF, aes(x=ID,y=log_absolu,label=ID)) + geom_point(size=0.5) + geom_text(size=2.5) ; p2= ggplot(tab_lm_EF, aes(x=log_absolu,label=ID)) + geom_histogram() ; plot_grid(p1, p2)
  }
  shapiro.test(EF.log)
  #EF=µEF+D+FQ1+FQ2+FQ1*FQ2+J+J*FQ1+J*FQ2+J*FQ1*FQ2+e #D*FQ1+D*FQ2+D*FQ1*FQ2 => effet dosage sur traitement ??
  ...
  if (extendedResults==TRUE){
    model.EF.ATB_type <- aov(log_absolu~ATB_type, data = tab_lm_EF_Dunnett)
    shapiro.test(model.EF.ATB_type$residuals)
    summary(model.EF.ATB_type)
  }
  #boxplot(log_absolu~ATB_type, data = tab_lm_EF_Dunnett)
  print(ggplot(tab_lm_EF_Dunnett, aes(x=ATB_type, y=log_absolu,color=ATB_type)) +
    geom_boxplot(outlier.colour="black", outlier.shape=16,outlier.size=2, notch=FALSE) + 
    scale_color_manual(values=c("#c0392b", "#E69F00", "#56B4E9", "#999999")) + 
    geom_dotplot(binaxis='y', stackdir='center',
                   position=position_dodge(1)))
  ...
  print(DunnettTest(x=tab_lm_EF_Dunnett$log_absolu, g=tab_lm_EF_Dunnett$ATB_type))
  ...
  if (extendedResults==TRUE){
    model.EF.Dosage <- aov(log_absolu~Dosage, data = tab_lm_EF_Dunnett)
    shapiro.test(model.EF.Dosage$residuals)
    print(summary(model.EF.Dosage))
  }
  #boxplot(log_absolu~Dosage, data = tab_lm_EF_Dunnett)
  print(ggplot(tab_lm_EF_Dunnett, aes(x=Dosage, y=log_absolu,color=Dosage)) + geom_boxplot(outlier.colour="black", outlier.shape=16,outlier.size=2, notch=FALSE) + scale_color_grey() + theme_classic() +geom_dotplot(binaxis='y', stackdir='center',
                   position=position_dodge(1)))

  library(DescTools)
  print(DunnettTest(x=tab_lm_EF_Dunnett$log_absolu, g=tab_lm_EF_Dunnett$Dosage))
}
```
...
```{r 16S, warning=FALSE}
results_log(PCR_18_16S_tri,metadata)
```
</br>

## qnrA  
</br>
```{r qnrA, warning=FALSE}
